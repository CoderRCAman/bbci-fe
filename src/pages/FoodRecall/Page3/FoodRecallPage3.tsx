...
import { 
    IFoodRecallEntry, IFoodRecallIngredient, 
    loadRecallData, saveRecallData, checkHabitEditEligibility 
...
export default function FoodRecallEntryPage() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const canEdit = await checkHabitEditEligibility(db, masterId, tabId);
    ...
    const handleFoodEntryChange = (id: string, field: keyof IFoodRecallEntry, value: any) => {
        setFoodLog(foodLog.map((entry) => entry.id === id ? { ...entry, [field]: value } : entry));
    };

    const handleAddIngredient = (foodId: string) => {
        setFoodLog(foodLog.map((food) => {
            if (food.id === foodId) {
    ...
    const handleIngredientChange = (foodId: string, ingId: string, field: keyof IFoodRecallIngredient, value: any) => {
        setFoodLog(foodLog.map((food) => {
            if (food.id === foodId) {
                return { ...food, ingredients: food.ingredients.map((ing:any) => ing.id === ingId ? { ...ing, [field]: value } : ing) };
            }
            return food;
        }));
    };
    ...
    const handleRemoveIngredient = (foodId: string, ingId: string) => {
        setFoodLog(foodLog.map((food) => {
            if (food.id === foodId) {
                return { ...food, ingredients: food.ingredients.filter((ing:any) => ing.id !== ingId) };
            }
            return food;
        }));
    };
                ...
                <main className="space-y-10 p-2">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow border">
                        <div className="flex justify-between items-center mb-4 border-b pb-2">
                            <h2 className="text-xl font-semibold text-gray-700">24-Hour Food Log (7.3)</h2>
                        ...
                        <div className="space-y-6">
                            {foodLog.map((entry, index) => (
                                <div key={entry.id} className="border-2 border-cyan-100 rounded-lg p-4 bg-cyan-50/50 relative">
                                    ...
                                    <button onClick={() => handleRemoveFoodEntry(entry.id)} disabled={!isEditable} className="absolute top-2 right-2 text-red-500 hover:text-red-700 font-bold text-xl px-2 py-0 disabled:text-gray-300" title="Remove Dish">
                                        &times;
                                    </button>
                    ...
                    <div className="mt-10 flex justify-between gap-2">
                        ...
                        <Link to={`/food-recall/page2?master_id=${masterId}&user_id=${userId}`}>
