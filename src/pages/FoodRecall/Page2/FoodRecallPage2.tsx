...
import { IonContent, IonPage, IonAlert, IonSelect, IonSelectOption } from "@ionic/react";
import Header from "../../../components/Header";
import { 
    IFoodHabitMaster, IFoodHabitFat, 
    generateDefaultHabitState, loadHabitData, saveHabitData, checkHabitEditEligibility 
...
const CustomRadio = ({ id, name, value, checked, onChange, label, disabled = false }: { id: string; name: string; value: any; checked: boolean; onChange: any; label: string; disabled?: boolean; }) => (
    <div className="flex items-center">
        <input id={id} type="radio" name={name} value={value} checked={checked} onChange={onChange} disabled={disabled} className="w-4 h-4 text-cyan-600 border-gray-300 focus:ring-cyan-500" />
        <label htmlFor={id} className={`ml-2 text-sm font-medium ${disabled ? 'text-gray-400' : 'text-gray-700 cursor-pointer'}`}>{label}</label>
    </div>
);
const PREP_METHODS = ["Shallow Frying", "Deep Frying", "Boiling", "Steaming", "Sauting", "Grill/Barbeque"] as const;
type PrepMethodKey = 'method_shallow_frying' | 'method_deep_frying' | 'method_boiling' | 'method_steaming' | 'method_sauting' | 'method_grill_bbq';
...
export default function FoodHabitPage() {
        ...
        const loadOrCreateData = async () => {
            ...
            try {
                ...
                if (existingData) {
                    ...
                    const canEdit = await checkHabitEditEligibility(db, existingData.master.id, tabId);
                    setIsEditable(canEdit);
                    if (!canEdit) {
                        setAlert({ show: true, header: "Read Only", message: "This record was created on another device and cannot be edited here." });
                    }
                } else if (userId) {
                    // CREATE MODE
    ...
    const handleSave = async () => {
        ...
        setIsLoading(true);
        try {
            await saveHabitData(db, sqlite, master, fats);
            setIsLoading(false);
            setAlert({ show: true, header: "Success", message: "Food Habit data has been saved." });
    ...
    if (isLoading) {
        return <IonPage><Header title="Loading..." /><IonContent><div className="p-4">Loading data...</div></IonContent></IonPage>;
    }

    if (!master) {
        return <IonPage><Header title="Error" /><IonContent><div className="p-4">Could not load or create habit record. Please go back to Page 1 and select a patient.</div></IonContent></IonPage>;
    }
                ...
                <main className="space-y-10 p-2">
                            ...
                            <div className="space-y-4">
                                {fats.map((fat, index) => (
                                    <div key={fat.id} className="border border-dashed border-gray-300 rounded-md p-3 bg-white relative">
                                        <div className="flex justify-between items-start mb-2"><h4 className="text-md font-semibold text-gray-800">Fat/Oil {index + 1}</h4>
                                ...
                                <div key={method} className="grid grid-cols-4 items-center py-2 border-b border-gray-100 last:border-b-0">
                                    ...
                                    {['0', '1', '2'].map((frequency) => (
                                        <div key={`${method}-${frequency}`} className="flex justify-center">
                                            <input type="radio" id={`prep-${method}-${frequency}`} name={`prepMethod-${method}`} value={frequency}
                                                checked={master[key] === frequency} disabled={!isEditable}
                                                onChange={(e) => handleMasterChange(key, e.target.value as PrepFrequencyValue)}
                                                className="w-4 h-4 text-cyan-600 border-gray-300 focus:ring-cyan-500" />
                                        </div>
                                    ))}
                                </div>
                    ...
                    <div className="bg-white p-6 rounded-lg shadow border">
                        ...
                        <div className="mb-4">
                            ...
                            <input type="number" id="family_sharing" value={master.family_sharing} onChange={(e) => handleMasterChange('family_sharing', e.target.value)} placeholder="Enter number" disabled={!isEditable} className="w-full p-2 border rounded-md disabled:bg-gray-100" />
                        </div>
                        <div className="mb-4">
                            ...
                            <input type="number" id="meals_per_day" value={master.meals_per_day} onChange={(e) => handleMasterChange('meals_per_day', e.target.value)} placeholder="Enter number" disabled={!isEditable} className="w-full p-2 border rounded-md disabled:bg-gray-100" />
                        </div>
                        <div className="mb-4">
                            ...
                            <IonSelect
                                value={waterSupply}
                                multiple={true}
                                onIonChange={(e) => handleMasterChange('water_supply_json', JSON.stringify(e.detail.value))}
                    ...
                    <div className="mt-10 flex justify-between gap-2">
                        <Button label="Save & Continue to Recall (7.3)" severity="success" className="px-10 py-2" onClick={handleSave} disabled={!isEditable || isLoading} />
                        <Link to={`/food1`}>
                            <Button label="Back to Patient List" className="px-5 py-2 rounded" />
                        </Link>
                    </div>
                </main>
