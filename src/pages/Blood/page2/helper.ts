...
export const ErrorDetectionBloodSample = (
  data: BLOOD_SAMPLE
): string | null => {
  ...
  if (
    !Array.isArray(data.collection_tubes) ||
    data.collection_tubes.length === 0
  )
  ...
  for (let i = 0; i < data.collection_tubes.length; i++) {
    ...
    if (!tube.identification_code_tube)
      return `Missing: Identification Code Tube`;
